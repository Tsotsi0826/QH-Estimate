...
// Version with ENHANCED console logging for debugging Edit/Delete functionality.
...
async function loadAndRenderModules() {
    ...
    try {
        ...
        if (!loadedModules || loadedModules.length === 0) {
             ...
             loadedModules = restoreModuleOrderFromBackup() || []; // Use empty array if backup fails
    ...
    notesModuleData = {
        ...
        order: notesModuleData.order !== undefined ? notesModuleData.order : -1
    };
    ...
    appData.modules = loadedModules.map(mod => ({
        ...
        renderTemplate: function(client) {
            ...
            if (mod.id === 'notes') {
                 ...
                 return `<h3>Project Notes</h3><textarea id="project-notes" rows="10" style="width: 100%; padding: 10px;" placeholder="Enter project notes here...">${notesText}</textarea><button class="btn module-save-btn" data-module="notes" style="margin-top: 10px;">Save Notes</button>`;
            }
    ...
    // saveModuleStructure(); // Avoid saving structure on initial load unless necessary
}
...
function renderModuleList(modules) {
    ...
    setupModuleElementEventListeners(); // Attach listeners to newly created elements
...
function setupModuleElementEventListeners() {
     document.querySelectorAll('.module-item').forEach(moduleElement => {
         ...
         moduleElement.dataset.listenersAttached = 'true';
     });
}
...
function setupAddModuleButton() {
    ...
    if (addModuleBtn) {
        addModuleBtn.addEventListener('click', () => {
            ...
            appData.modules.filter(m => m.type === 'header').sort((a,b) => a.name.localeCompare(b.name))
                .forEach(header => {
            ...
            document.getElementById('new-module-name').value = ''; moduleTypeSelect.value = 'regular'; parentHeaderGroup.style.display = 'block'; parentHeaderSelect.value = 'null'; document.getElementById('new-module-requires-client').checked = true; modalOverlay.style.display = 'flex';
        });
    }
    if (moduleTypeSelect) { moduleTypeSelect.addEventListener('change', function() { parentHeaderGroup.style.display = this.value === 'regular' ? 'block' : 'none'; }); }
    ...
    if (saveNewModuleBtn) { const newSaveBtn = saveNewModuleBtn.cloneNode(true); saveNewModuleBtn.parentNode.replaceChild(newSaveBtn, saveNewModuleBtn); newSaveBtn.addEventListener('click', addNewModule); }
}

function addNewModule() {
    const moduleNameInput = document.getElementById('new-module-name'); const moduleTypeSelect = document.getElementById('new-module-type'); const parentHeaderSelect = document.getElementById('parent-header-select'); const requiresClientCheckbox = document.getElementById('new-module-requires-client');
    const moduleName = moduleNameInput.value.trim(); const moduleType = moduleTypeSelect.value; const parentId = moduleType === 'header' ? null : (parentHeaderSelect.value === 'null' ? null : parentHeaderSelect.value); const requiresClient = requiresClientCheckbox.checked;
    ...
    if (appData.modules.some(m => m.id === moduleId)) { alert(`Module ID "${moduleId}" already exists.`); return; }
    let order = 0; const siblings = appData.modules.filter(m => m.parentId === parentId); if (siblings.length > 0) order = Math.max(...siblings.map(m => m.order ?? -1)) + 1;
    const newModuleData = { id: moduleId, name: moduleName, requiresClient: requiresClient, type: moduleType, parentId: parentId, order: order, renderTemplate: function(client) { return `<h3>${moduleName}</h3><p>Default content.</p>`; }, saveData: function() { return {}; } };
    console.log("Adding new module:", newModuleData); appData.modules.push(newModuleData); renderModuleList(appData.modules); saveModuleStructure(); document.getElementById('add-module-modal-overlay').style.display = 'none'; alert(`Module "${moduleName}" created.`);
}

// --- Module Structure Saving ---
function saveModuleStructure() {
    console.log("[Dashboard] Saving module structure");
     const modulesToSave = appData.modules.map(module => ({ id: module.id, name: module.name, requiresClient: module.requiresClient, type: module.type || 'regular', parentId: module.parentId, order: module.order ?? 0 }));
    ...
    if (window.ConstructionApp.Firebase) {
        window.ConstructionApp.Firebase.saveModules(modulesToSave).then(success => { if (success) { console.log("[Dashboard] Module structure saved to Firebase."); sessionStorage.setItem('moduleOrder', JSON.stringify(modulesToSave)); console.log("[Dashboard] Module structure backup saved to sessionStorage."); } else { console.warn("[Dashboard] Firebase.saveModules reported failure."); } }).catch(error => { console.error("[Dashboard] Error in saveModules promise chain:", error); sessionStorage.setItem('moduleOrder', JSON.stringify(modulesToSave)); console.warn("[Dashboard] Saved structure to sessionStorage backup due to Firebase error."); });
    } else { console.warn("[Dashboard] Firebase not available, saving structure to sessionStorage only"); sessionStorage.setItem('moduleOrder', JSON.stringify(modulesToSave)); }
}

function restoreModuleOrderFromBackup() {
    const savedOrder = sessionStorage.getItem('moduleOrder');
    if (savedOrder) {
        try { const orderData = JSON.parse(savedOrder); console.log("[Dashboard] Restoring structure from backup:", orderData.length, "modules"); return orderData.map(mod => ({ ...mod, type: mod.type || 'regular', parentId: (mod.parentId === undefined || mod.parentId === 'null') ? null : mod.parentId, order: mod.order ?? 0 })); }
        catch (error) { console.error("[Dashboard] Error parsing backup:", error); sessionStorage.removeItem('moduleOrder'); }
    } else { console.warn("[Dashboard] No backup found"); } return null;
}
...
function setupDragAndDrop() {
    const container = document.getElementById('modules-container');
    container.removeEventListener('dragstart', handleDragStart); container.addEventListener('dragstart', handleDragStart); container.removeEventListener('dragover', handleDragOver); container.addEventListener('dragover', handleDragOver); container.removeEventListener('dragleave', handleDragLeave); container.addEventListener('dragleave', handleDragLeave); container.removeEventListener('drop', handleDrop); container.addEventListener('drop', handleDrop); container.removeEventListener('dragend', handleDragEnd); container.addEventListener('dragend', handleDragEnd);
    // console.log("[Dashboard] Drag and Drop listeners attached."); // Reduced logging noise
}
function handleDragStart(e) {
     const target = e.target.closest('.module-item'); if (!target || !target.draggable) { e.preventDefault(); return; } globalDraggedItem = target; e.dataTransfer.setData('text/plain', target.dataset.moduleId); e.dataTransfer.effectAllowed = 'move'; setTimeout(() => { if (globalDraggedItem) globalDraggedItem.classList.add('dragging'); }, 0); console.log("Drag Start:", target.dataset.moduleId);
}
function handleDragOver(e) {
     e.preventDefault(); e.dataTransfer.dropEffect = 'move'; const targetElement = e.target.closest('.module-item'); if (!targetElement || targetElement === globalDraggedItem) { clearDropIndicators(); dragOverElement = null; dropIndicator = null; return; } if (targetElement !== dragOverElement) { clearDropIndicators(); dragOverElement = targetElement; } const rect = targetElement.getBoundingClientRect(); const yOffset = e.clientY - rect.top; const dropZoneHeight = rect.height; const targetIsHeader = targetElement.dataset.moduleType === 'header'; const canDropOnHeader = targetIsHeader && globalDraggedItem?.dataset.moduleType !== 'header'; const topThreshold = dropZoneHeight * 0.3; const bottomThreshold = dropZoneHeight * 0.7; let currentIndicator = null; if (canDropOnHeader && yOffset > topThreshold && yOffset < bottomThreshold) currentIndicator = 'middle'; else if (yOffset <= topThreshold) currentIndicator = 'top'; else currentIndicator = 'bottom'; if (currentIndicator !== dropIndicator) { targetElement.classList.remove('drag-over-top', 'drag-over-bottom', 'drag-over-middle'); dropIndicator = currentIndicator; if (dropIndicator === 'middle') targetElement.classList.add('drag-over-middle'); else if (dropIndicator === 'top') targetElement.classList.add('drag-over-top'); else if (dropIndicator === 'bottom') targetElement.classList.add('drag-over-bottom'); }
}
function handleDragLeave(e) {
     const targetElement = e.target.closest('.module-item'); if (!targetElement) return; const relatedTarget = e.relatedTarget ? e.relatedTarget.closest('.module-item') : null; if (targetElement === dragOverElement && relatedTarget !== dragOverElement) { clearDropIndicators(targetElement); if (!targetElement.contains(e.relatedTarget)) { dragOverElement = null; dropIndicator = null; } }
}
function handleDrop(e) {
    e.preventDefault(); if (!globalDraggedItem || !dragOverElement || globalDraggedItem === dragOverElement || !dropIndicator) { clearDropIndicators(); return; } console.log(`Drop ${globalDraggedItem.dataset.moduleId} onto ${dragOverElement.dataset.moduleId}, indicator: ${dropIndicator}`); const draggedId = globalDraggedItem.dataset.moduleId; const targetId = dragOverElement.dataset.moduleId; const draggedModuleIndex = appData.modules.findIndex(m => m.id === draggedId); const targetModuleIndex = appData.modules.findIndex(m => m.id === targetId); if (draggedModuleIndex === -1 || targetModuleIndex === -1) { console.error("DnD Error: Module not found in appData."); clearDropIndicators(); return; } const draggedModule = appData.modules[draggedModuleIndex]; const targetModule = appData.modules[targetModuleIndex]; let newParentId = null; let targetPositionIndex = -1; if (dropIndicator === 'middle' && targetModule.type === 'header') { newParentId = targetModule.id; targetPositionIndex = targetModuleIndex + 1; console.log(`Setting parent of ${draggedId} to ${newParentId}`); } else if (dropIndicator === 'bottom') { newParentId = targetModule.parentId; targetPositionIndex = targetModuleIndex + 1; console.log(`Inserting ${draggedId} after ${targetId} (parent: ${newParentId})`); } else { newParentId = targetModule.parentId; targetPositionIndex = targetModuleIndex; console.log(`Inserting ${draggedId} before ${targetId} (parent: ${newParentId})`); } draggedModule.parentId = newParentId; appData.modules.splice(draggedModuleIndex, 1); if (draggedModuleIndex < targetPositionIndex) targetPositionIndex--; appData.modules.splice(targetPositionIndex, 0, draggedModule); recalculateModuleOrder(); renderModuleList(appData.modules); saveModuleStructure(); clearDropIndicators();
}
function handleDragEnd(e) {
     if (globalDraggedItem) globalDraggedItem.classList.remove('dragging'); clearDropIndicators(); globalDraggedItem = null; dragOverElement = null; dropIndicator = null; console.log("Drag End");
}
function clearDropIndicators(element) {
     const selector = '.module-item.drag-over-top, .module-item.drag-over-bottom, .module-item.drag-over-middle'; const elementsToClear = element ? [element] : document.querySelectorAll(selector); elementsToClear.forEach(el => { if (el) el.classList.remove('drag-over-top', 'drag-over-bottom', 'drag-over-middle'); });
}
function recalculateModuleOrder() {
    console.log("Recalculating module order..."); const orderCounters = {}; appData.modules.forEach(module => { const parentKey = module.parentId === null ? 'null' : module.parentId; if (orderCounters[parentKey] === undefined) orderCounters[parentKey] = 0; module.order = orderCounters[parentKey]++; }); console.log("Order recalculation complete.");
}

// --- Other UI Functions ---
function setupDropdownMenus() {
     const container = document.getElementById('modules-container');
     container.removeEventListener('click', handleDropdownClick); // Ensure no duplicates
     container.addEventListener('click', handleDropdownClick);
     document.removeEventListener('click', handleGlobalClickForDropdowns); // Ensure no duplicates
     document.addEventListener('click', handleGlobalClickForDropdowns);
     console.log("[Dashboard] Dropdown listeners attached via delegation.");
}

// Handler for clicks within the modules container (for dropdowns)
function handleDropdownClick(e) {
     // console.log("Dropdown container clicked. Target:", e.target); // Basic log
     ...
     if (icon) {
         ...
         } else { console.log("Dropdown menu not found within icon."); }

     } else if (editBtn) {
         // ** NEW LOG **
         console.log("DEBUG: Edit button detected via closest('.edit-module')");
         e.stopPropagation();
         const moduleItem = editBtn.closest('.module-item');
         if (moduleItem) {
             console.log("DEBUG: Calling editModule for:", moduleItem.dataset.moduleId);
             editModule(moduleItem);
         } else { console.log("DEBUG: Could not find parent module item for edit button."); }
         closeAllDropdowns();

     } else if (deleteBtn) {
         // ** NEW LOG **
         console.log("DEBUG: Delete button detected via closest('.delete-module')");
         e.stopPropagation();
         const moduleItem = deleteBtn.closest('.module-item');
         if (moduleItem) {
             console.log("DEBUG: Calling deleteModule for:", moduleItem.dataset.moduleId);
             deleteModule(moduleItem);
         } else { console.log("DEBUG: Could not find parent module item for delete button."); }
         closeAllDropdowns();
     }
}
...
function editModule(moduleElement) {
     console.log("DEBUG: editModule function started."); // ** NEW LOG **
     ...
     console.log("DEBUG: Editing module:", currentModule); // ** NEW LOG **

     // ** NEW LOG **
     console.log("DEBUG: About to show prompt for new name...");
     const newName = prompt(`Edit module name:`, currentName);
     console.log("DEBUG: Prompt returned:", newName); // ** NEW LOG **

     if (newName && newName.trim() !== '' && newName !== currentName) {
        const finalNewName = newName.trim();
        console.log("DEBUG: New name accepted:", finalNewName);
        currentModule.name = finalNewName;
        const nameSpan = moduleElement.querySelector('.module-name');
        if (nameSpan) { nameSpan.textContent = finalNewName; console.log("DEBUG: DOM name updated."); }
        else { console.log("DEBUG: Warning: Could not find name span in DOM."); }
        saveModuleStructure();
...
function deleteModule(moduleElement) {
     console.log("DEBUG: deleteModule function started."); // ** NEW LOG **
     const moduleId = moduleElement?.dataset?.moduleId;
     ...
     if (moduleToDelete.type === 'header' && children.length > 0) { confirmMessage += `\n\nWARNING: This is a header with ${children.length} sub-module(s). Deleting it will also delete ALL descendants. This cannot be undone.`; }
     else { confirmMessage += `\nThis cannot be undone.`; }

    // ** NEW LOG **
    console.log("DEBUG: About to show confirm dialog...");
    const confirmed = confirm(confirmMessage);
    console.log("DEBUG: Confirm dialog returned:", confirmed); // ** NEW LOG **

    if (confirmed) {
         console.log("DEBUG: Deletion confirmed by user.");
         ...
         console.log("DEBUG: appData filtered. Remaining:", appData.modules.length);

         recalculateModuleOrder(); renderModuleList(appData.modules); saveModuleStructure();
         alert(`Module "${moduleName}" ${idsToDelete.size > 1 ? 'and its sub-modules ' : ''}deleted.`);
    } else { console.log("DEBUG: Deletion cancelled by user."); }
}

// --- Client Management & Dashboard Update --- (No changes in this section)
...
function updateDebugPanel() { const debugPanel = document.getElementById('debug-panel'); if (!debugPanel || debugPanel.style.display !== 'block') return; const navigationState = sessionStorage.getItem('navigationState'); const currentClient = window.ConstructionApp.ClientManager.getCurrentClient(); const storedClientStr = sessionStorage.getItem('currentClient'); const moduleOrderStr = sessionStorage.getItem('moduleOrder'); let debugInfo = `<strong>Nav State:</strong> ${navigationState || 'None'}<br><strong>Current Client:</strong> ${currentClient ? currentClient.name : 'None'} (ID: ${currentClient?.id || 'N/A'})<br><strong>Client in sessionStorage:</strong> ${storedClientStr ? 'Present' : 'None'}<br><strong>Modules in appData:</strong> ${appData.modules.length}<br><strong>Module Order in Backup:</strong> ${moduleOrderStr ? 'Present' : 'None'}<br><hr>`; if (currentClient && currentClient.moduleData) { debugInfo += '<strong>Client Module Data (Costs):</strong><br>'; let totalCost = 0; Object.entries(currentClient.moduleData).forEach(([moduleId, moduleVData]) => { const moduleData = moduleVData?.data ?? moduleVData ?? {}; let moduleCost = 0; if (moduleData?.totalCost !== undefined) moduleCost = moduleData.totalCost; else if (moduleData?.items) moduleCost = window.ConstructionApp.ModuleUtils.calculateModuleTotal(moduleData.items); totalCost += parseFloat(moduleCost) || 0; debugInfo += `- ${moduleId}: Cost: ${window.ConstructionApp.ModuleUtils.formatCurrency(moduleCost)}<br>`; }); debugInfo += `<strong>Calculated Total Cost:</strong> ${window.ConstructionApp.ModuleUtils.formatCurrency(totalCost)}<br><hr>`; } debugInfo += '<strong>Module Structure (appData):</strong><br><pre style="max-height: 150px; overflow-y: auto; border: 1px solid #555; padding: 5px; font-size: 11px;">'; debugInfo += JSON.stringify(appData.modules.map(m => ({id: m.id, name: m.name, type: m.type, parentId: m.parentId, order: m.order})), null, 1); debugInfo += '</pre>'; debugPanel.innerHTML = debugInfo; }
 ...
 function setupDropdownForModule(moduleElement) { /* Placeholder - delegation handles actions */ }
