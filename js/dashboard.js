// js/dashboard.js
// Added logic to make sidebar headers start collapsed by default.
// Change is within loadAndRenderModules function.
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadAndRenderModules().then(() => {
        ...
        // Assign the callback BEFORE initApp is called
        ...
        initApp(); // Determine initial client state (will now trigger the callback)
 ...
 * Initializes default collapse state for headers.
...
async function loadAndRenderModules() {
    ...
    // --- ADDED: Initialize header collapse state to collapsed by default ---
    console.log("DEBUG: Initializing headerCollapseState to collapsed by default.");
    headerCollapseState = {}; // Clear previous state just in case
    appData.modules.forEach(module => {
        if (module.type === 'header') {
            headerCollapseState[module.id] = true; // Set header to collapsed
        }
    });
    console.log("DEBUG: Initial headerCollapseState:", JSON.stringify(headerCollapseState));
    // --- END ADDED ---

    // Render the sidebar list (will now use the pre-populated headerCollapseState)
    ...
    function renderLevel(parentId, level) {
        sortedModules
            ...
            .forEach(module => {
                ...
                // Check the state variable to see if this header is collapsed
                const isCollapsed = headerCollapseState[module.id] === true;
                // Render children only if it's not a header OR if it's a header that is NOT collapsed
...
function createModuleElement(moduleData, level = 0) {
    ...
    if (moduleData.type === 'header') {
        ...
        // Check the state variable to add the 'collapsed' class if needed
...
function setupDashboardTileListeners() { const container = E('module-tiles'); if (!container) return; container.removeEventListener('click', handleTileClick); container.addEventListener('click', handleTileClick); }
...
function updateTotalProjectCost() {
    ...
    const formatted = window.ConstructionApp?.ModuleUtils?.formatCurrency(total) ?? `R${total.toFixed(2)}`; const el = E('total-project-cost'); if (el) el.textContent = `Total Project Cost: ${formatted}`;
}

// --- Debug Panel --- (No changes needed here)
function setupDebugPanel() { const dp = E('debug-panel'); let btn = E('debug-toggle-btn'); if (!dp) return; if (!btn) { btn = document.createElement('button'); btn.textContent = 'Debug'; btn.id = 'debug-toggle-btn'; btn.style.cssText = `position:fixed; bottom:10px; right:10px; z-index:10000; padding:5px 10px; background:#333; color:white; border:none; border-radius:3px; cursor:pointer; opacity:0.8; font-size:12px;`; document.body.appendChild(btn); } btn.style.display = 'block'; btn.removeEventListener('click', toggleDebugPanel); btn.addEventListener('click', toggleDebugPanel); }
function toggleDebugPanel() { const dp = E('debug-panel'), btn = E('debug-toggle-btn'); if (!dp || !btn) return; const vis = dp.style.display === 'block'; dp.style.display = vis ? 'none' : 'block'; btn.textContent = vis ? 'Debug' : 'Hide Debug'; if (!vis) updateDebugPanel(); }
function updateDebugPanel() { const dp = E('debug-panel'); if (!dp || dp.style.display !== 'block') return; const navState = sessionStorage.getItem('navigationState'); const client = window.ConstructionApp?.ClientManager?.getCurrentClient(); const sessStr = sessionStorage.getItem('currentClient'); const modOrdStr = sessionStorage.getItem('moduleOrder'); let sessCli = null; try { sessCli = JSON.parse(sessStr || 'null'); } catch(e){} let info = `<strong>Nav:</strong> ${navState||'N/A'}<br><strong>Client(Mgr):</strong> ${client?client.name:'N/A'}<br><strong>Client(Sess):</strong> ${sessCli?'Yes':'No'}<br><hr><strong>Mods:</strong> ${appData.modules?.length||0}<br><strong>Backup:</strong> ${modOrdStr?'Yes':'No'}<br><strong>Collapse:</strong> ${JSON.stringify(headerCollapseState)}<hr>`; if (client?.moduleData) { info += '<strong>Costs:</strong><br>'; let total = 0; Object.entries(client.moduleData).forEach(([id, vData]) => { const mData = vData?.data ?? vData ?? {}; let cost = 0; if (mData.totalCost !== undefined) cost = mData.totalCost; else if (mData.items) cost = window.ConstructionApp?.ModuleUtils?.calculateModuleTotal(mData.items) ?? 0; total += parseFloat(cost) || 0; info += `- ${id}: ${window.ConstructionApp?.ModuleUtils?.formatCurrency(cost) ?? 'N/A'}<br>`; }); info += `<strong>Total:</strong> ${window.ConstructionApp?.ModuleUtils?.formatCurrency(total) ?? 'N/A'}<hr>`; } info += '<strong>Struct(Top 5):</strong><br><pre style="max-height:100px; overflow-y:auto; border:1px solid #555; padding:3px; font-size:10px;">'; info += JSON.stringify(appData.modules?.slice(0, 5).map(m => ({id:m.id, n:m.name, t:m.type, p:m.parentId, o:m.order})) || [], null, 1); info += '</pre>'; dp.innerHTML = info; }
