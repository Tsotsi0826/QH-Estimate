// js/dashboard.js
// Reverted to the version confirmed working after fixing the callback timing issue.
// Does NOT include the 'Reports' module link. Headers start expanded.
...
function setupModuleSearch() {
    ...
    const filterModules = () => {
        // Access global appData
        const modules = window.appData?.modules;
        if (typeof appData === 'undefined' || !modules) {
            console.error("[Dashboard] appData or appData.modules not available for search.");
            return;
        }
        // Access global renderModuleList
        const renderListFunc = window.ConstructionApp?.DashboardUtils?.renderModuleList;
        if (typeof renderListFunc !== 'function') {
             console.error("[Dashboard] renderModuleList function not available for search.");
        }
        ...
        if (searchTerm === '') {
            ...
            if (renderListFunc) {
                renderListFunc(modules); // Re-render to fix potential collapse state overrides
            }
            return;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadAndRenderModules().then(() => {
        ...
        setupModuleSearch(); // Call the setup function
        setupDragAndDrop();
...
async function loadAndRenderModules() {
    ...
    // NOTE: Does NOT dynamically add 'Reports' module in this version
 ...
 */
function renderModuleList(modules) { // Keep this function here
    const container = document.getElementById('modules-container');
...
// Expose renderModuleList globally for sidebar-search.js (if sidebar-search is used)
 ...
 */
function createModuleElement(moduleData, level = 0) { // Keep this function here
...
// Expose createModuleElement globally if needed by other future modules (optional)
window.ConstructionApp.DashboardUtils.createModuleElement = createModuleElement;
...
function clearModuleData(id) { const cli = window.ConstructionApp?.ClientManager?.getCurrentClient(); if (!cli) { window.ConstructionApp?.ModuleUtils?.showErrorMessage("No client."); return; } if (cli.moduleData?.[id]) { console.log(`Clearing ${id}`); window.ConstructionApp.ClientManager.saveModuleData(id, null, (ok, err) => { if (ok) { console.log("Data cleared."); if (cli.moduleData) { delete cli.moduleData[id]; window.ConstructionApp.ClientManager.setCurrentClient({ ...cli }); } window.ConstructionApp?.ModuleUtils?.showSuccessMessage(`Cleared.`); } else { console.error(`Err: ${err}`); window.ConstructionApp?.ModuleUtils?.showErrorMessage(`Err: ${err}`); } }); } else window.ConstructionApp?.ModuleUtils?.showSuccessMessage(`No data.`); }
