...
function setupModuleSearch() {
    ...
    const filterModules = () => {
        ...
        allModuleElements.foranEach(moduleEl => {
...
async function loadAndRenderModules() {
    ...
    // Set default collapsed state for all headers
    appData.modules.forEach(module => {
        if (module.type === 'header') {
            if (headerCollapseState[module.id] === undefined) {
                headerCollapseState[module.id] = true; // Headers start collapsed by default
            }
        }
    });
...
function createClientModal(type) {
    ...
    if (type === 'new') {
        modal.innerHTML = `
            <div class="modal-header"><h2 class="modal-title">New Client</h2><span class="modal-close" data-modal-id="${overlayId}">×</span></div>
        ...
        modal.innerHTML = `
            <div class="modal-header"><h2 class="modal-title">Open Client</h2><span class="modal-close" data-modal-id="${overlayId}">×</span></div>
            <div class="modal-body">
